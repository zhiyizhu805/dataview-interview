...
const margin = { top: 20, right: 10, bottom: 30, left: 20 },
  ...
  barPadding = 7,
  graph_misc = { ylabel: 4, xlabelH: 8, title: 9 };
...
function d3BarChart(datasetBarChart) {
  ...
  const xScale = d3
    .scaleLinear() // Barchart X axis scale
    .domain([0, defaultBarChart.length]) // Scale range from 0 to the length of data object
    .range([0, width]);

  const yScale = d3
    .scaleLinear() // Barchart y axis scale
    .domain([
      0,
      d3.max(defaultBarChart, function (d) {
        return d.value;
      }),
    ]) //Scale range from 0 to the maximum value of the default bar chart data
    .range([height, 0]);
  ...
  visualization
    ...
    .attr("x", function (d, i) {
      return (
        i * (width / defaultBarChart.length) +
        (width / defaultBarChart.length - barPadding) / 2
      );
    })
    .attr("y", function (d) {
